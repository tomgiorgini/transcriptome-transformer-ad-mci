...
DEFAULT_RESULT_DIR = ROOT / "results" / "training" / "default_run"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=["on", "off"],
        default="on",
        help="Enable or ablate the patient-expression TUPE attention contribution.",
    )
    ...
    if state_dicts and len(state_dicts) > 1:
        ...
        for state_dict in state_dicts:
            ...
            run_logits, run_labels, run_masks = collect_logits(
                model, gene_x, task_y, task_mask, batch_size, device, max_batches, mask_aware_heads
            )
...
def train_model(
    ...
    shared_params = [parameter for parameter in [*embedding_params, *encoder_params] if parameter.requires_grad]
    encoder_parameter_ids = {id(parameter) for parameter in [*embedding_params, *encoder_params]}
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for gate_name, gate_value in model.ppi_gate_values().items():
    ...
    return history, best_state, checkpoint_ensemble, {
        ...
        "final_ppi_gate_values": model.ppi_gate_values(),
...
@torch.no_grad()
def save_trained_embedding_artifacts(model: TxT, gene_names: list[str], result_dir: Path) -> list[dict[str, Any]]:
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        training_summary["best_ppi_gate_values"] = model.ppi_gate_values()
        torch.save(best_state, args.result_dir / "best_model.pt")
        ensemble_states = [entry["state"] for entry in checkpoint_ensemble]
        for rank, entry in enumerate(checkpoint_ensemble, start=1):
            torch.save(entry["state"], args.result_dir / f"ensemble_checkpoint_rank{rank}.pt")
    ...
    pd.DataFrame(history).to_csv(args.result_dir / "training_log.csv", index=False)
    gradient_columns = [
        column
        for column in pd.DataFrame(history).columns
        if column == "epoch" or column.startswith("grad_") or column.startswith("ppi_gate_")
    ]
    if len(gradient_columns) > 1:
        pd.DataFrame(history)[gradient_columns].to_csv(args.result_dir / "gradient_diagnostics.csv", index=False)
