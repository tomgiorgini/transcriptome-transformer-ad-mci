...
METRIC_COLS = ["pr_auc", "roc_auc", "accuracy", "balanced_accuracy", "macro_f1", "weighted_f1", "log_loss"]
...
def summarize(result_root: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    group_cols = [col for col in ["protocol", "scenario", "feature_set", "model"] if col in all_metrics.columns]
    summary = (
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Summarize Kelly et al. 2023 reproduction results.")
    parser.add_argument("--result-root", type=Path, default=Path("results/SOTA/nature-2023"))
