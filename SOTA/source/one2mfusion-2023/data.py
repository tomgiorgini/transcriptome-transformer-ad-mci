...
def load_ad_mci_dataset(x_file: Path, y_file: Path) -> tuple[pd.DataFrame, pd.Series]:
    ...
    y = pd.Series(pd.to_numeric(merged["label"], errors="raise").astype(int).to_numpy(), index=merged["sample_id"], name="label")
    if sorted(y.unique().tolist()) != [0, 1]:
        raise ValueError(f"Expected binary labels [0, 1], found {sorted(y.unique().tolist())}.")

    gene_cols = [column for column in merged.columns if column not in {"sample_id", "label"}]
