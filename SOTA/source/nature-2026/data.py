...
def _metadata_sample_ids(path: Path) -> set[str]:
    if not path.exists():
    ...
    for col in ("sample_id", "geo_accession"):
        if col in metadata.columns:
            return set(metadata[col].astype(str).str.strip())
...
def infer_batch_labels(sample_ids: pd.Index, gse63060_metadata: Path, gse63061_metadata: Path) -> pd.Series:
    gse63060 = _metadata_sample_ids(gse63060_metadata)
    gse63061 = _metadata_sample_ids(gse63061_metadata)
    values: dict[str, str] = {}
    for sample_id in sample_ids.astype(str):
        gsm_id = sample_id.split("_", 1)[0].strip()
        if gsm_id in gse63060:
            values[sample_id] = "GSE63060"
        elif gsm_id in gse63061:
            values[sample_id] = "GSE63061"
        else:
            values[sample_id] = "unknown"
    batch = pd.Series(values, index=sample_ids, name="batch")
