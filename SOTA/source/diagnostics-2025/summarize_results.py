...
def summarize(result_root: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    group_cols = [col for col in ["protocol", "scenario", "feature_selector", "sampling", "model"] if col in all_metrics.columns]
    ...
    return all_metrics, summary.sort_values(["roc_auc_mean", "macro_f1_mean"], ascending=False)
